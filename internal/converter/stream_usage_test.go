...
func TestConvertChatChunkToResponsesSSE_CapturesUsage(t *testing.T) {
	...
	done := ConvertChatChunkToResponsesSSE(w, state, []byte(`{"id":"resp_1","object":"chat.completion.chunk","model":"gpt-4o","choices":[{"index":0,"delta":{"role":"assistant","content":""}}]}`))
	assert.False(t, done)

	// Content chunk
	done = ConvertChatChunkToResponsesSSE(w, state, []byte(`{"id":"resp_1","object":"chat.completion.chunk","model":"gpt-4o","choices":[{"index":0,"delta":{"content":"hello"},"finish_reason":"stop"}]}`))
	assert.False(t, done)

	// Final chunk with usage
	done = ConvertChatChunkToResponsesSSE(w, state, []byte(`{"id":"resp_1","object":"chat.completion.chunk","model":"gpt-4o","choices":[],"usage":{"prompt_tokens":100,"completion_tokens":50,"total_tokens":150}}`))
	assert.False(t, done)
...
func TestConvertResponsesLineToChat_CapturesUsage(t *testing.T) {
	...
	// response.created
	result := ConvertResponsesLineToChat(state, []byte(`data: {"type":"response.created","response":{"id":"resp_1","model":"test-model"}}`))
	assert.NotNil(t, result)
	assert.True(t, state.Started)

	// response.completed with usage
	result = ConvertResponsesLineToChat(state, []byte(`data: {"type":"response.completed","response":{"id":"resp_1","model":"test-model","usage":{"input_tokens":75,"output_tokens":30,"total_tokens":105}}}`))
	assert.NotNil(t, result)
...
func TestConvertAnthropicLineToChat_CapturesUsage(t *testing.T) {
	...
	result := ConvertAnthropicLineToChat(state, []byte(`data: {"type":"message_start","message":{"id":"msg_1","model":"claude-sonnet-4-5","usage":{"input_tokens":200}}}`))
	assert.NotNil(t, result)
	assert.Equal(t, 200, state.InputTokens)

	// message_delta with output_tokens
	result = ConvertAnthropicLineToChat(state, []byte(`data: {"type":"message_delta","delta":{"stop_reason":"end_turn"},"usage":{"output_tokens":80}}`))
	assert.NotNil(t, result)
...
func TestConvertAnthropicLineToResponses_CapturesUsage(t *testing.T) {
	...
	// message_start
	done := ConvertAnthropicLineToResponses(w, state, []byte(`data: {"type":"message_start","message":{"id":"msg_1","model":"claude-sonnet-4-5","usage":{"input_tokens":150}}}`))
	assert.False(t, done)
	...
	done = ConvertAnthropicLineToResponses(w, state, []byte(`data: {"type":"message_delta","delta":{"stop_reason":"end_turn"},"usage":{"output_tokens":60}}`))
	assert.True(t, done)
...
func TestConvertChatChunkToAnthropicSSE_CapturesUsage(t *testing.T) {
	...
	done := ConvertChatChunkToAnthropicSSE(w, state, []byte(`{"id":"chatcmpl-1","model":"gpt-4o","choices":[{"index":0,"delta":{"role":"assistant","content":""},"finish_reason":""}]}`))
	assert.False(t, done)

	// Content chunk
	done = ConvertChatChunkToAnthropicSSE(w, state, []byte(`{"id":"chatcmpl-1","model":"gpt-4o","choices":[{"index":0,"delta":{"content":"hello"},"finish_reason":""}]}`))
	assert.False(t, done)

	// Finish reason
	done = ConvertChatChunkToAnthropicSSE(w, state, []byte(`{"id":"chatcmpl-1","model":"gpt-4o","choices":[{"index":0,"delta":{},"finish_reason":"stop"}]}`))
	assert.False(t, done)

	// Usage chunk triggers deferred message_delta with real tokens
	done = ConvertChatChunkToAnthropicSSE(w, state, []byte(`{"id":"chatcmpl-1","model":"gpt-4o","choices":[],"usage":{"prompt_tokens":100,"completion_tokens":50,"total_tokens":150}}`))
	assert.False(t, done)
...
func TestConvertChatChunkToAnthropicSSE_OutputTokenTracking(t *testing.T) {
	...
	done = ConvertChatChunkToAnthropicSSE(w, state, []byte(`{"id":"chatcmpl-1","model":"gpt-4o","choices":[{"index":0,"delta":{"content":"hello world"},"finish_reason":""}]}`))
	assert.False(t, done)
...
func TestResponsesStreamState_NoUsage(t *testing.T) {
	...
	done = ConvertChatChunkToResponsesSSE(w, state, []byte(`{"id":"resp_1","object":"chat.completion.chunk","model":"gpt-4o","choices":[{"index":0,"delta":{"content":"hi"},"finish_reason":"stop"}]}`))
	assert.False(t, done)
...
func TestResponsesToChatState_NoUsage(t *testing.T) {
	...
	result := ConvertResponsesLineToChat(state, []byte(`data: {"type":"response.completed","response":{"id":"resp_1","model":"test"}}`))
	assert.NotNil(t, result)
